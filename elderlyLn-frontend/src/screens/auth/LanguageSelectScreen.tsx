...
// Language options shown to the user on the language select screen
...
export default function LanguageSelectScreen({ navigation }: Props) {
  ...
  // Selected app language when the screen loads.
  useEffect(() => {
    const current = i18n.language as Lang;
    if (current === "en" || current === "si" || current === "ta") setSelected(current);
  }, []);

  // Change the app language and navigate to the next screen.
  ...
  return (
    <View style={styles.container}>
      ...
      {options.map((opt) => { // Check if this option is the currently selected language.
